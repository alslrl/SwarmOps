/**
 * playwright-screenshots-ac12.spec.mjs
 *
 * Sub-AC 12.1: Playwright screenshot capture for AC12 evidence
 * Captures four named screenshots stored under artifacts/screenshots/:
 *
 *   empty_state_desktop.png     — initial dashboard, no simulation run yet
 *   loading_state_desktop.png   — simulation in progress, loading state visible
 *   completed_state_desktop.png — simulation complete, all results populated
 *   error_state_desktop.png     — SSE error event received, error message shown
 *
...
const PORT = 3115; // Unique port — no collision with other specs
...
const MOCK_COMPLETE_PAYLOAD = {
  ...
  artifact: {
    payload: {
      selected_strategy_id: 'ac12-strategy',
      holdout_uplift: 548900,
...
const MOCK_SSE_COMPLETE = buildSSEBody([
  {
    ...
    data: { iteration: 1, total: 2, candidates: [{ id: 'ac12-strategy', title: '트리클리닉 두피과학 기반 탈모 샴푸', price_krw: 28900 }] },
  },
  {
  ...
  {
    type: 'iteration_start',
    data: { iteration: 2, total: 2, candidates: [{ id: 'ac12-strategy-v2', title: '트리클리닉 전문가 설계 탈모 샴푸', price_krw: 29900 }] },
  },
  {
    type: 'archetype_evaluated',
...
// ── Screenshot 1: empty_state_desktop ────────────────────────────────────────

test('AC12 screenshot 1: empty_state_desktop', async ({ page }) => {
  ...
  console.log('✅ Saved: empty_state_desktop.png');
});

// ── Screenshot 2: loading_state_desktop ──────────────────────────────────────

test('AC12 screenshot 2: loading_state_desktop', async ({ page }) => {
  await page.goto(BASE_URL);
  await waitForFixtures(page);

  // Wait for particle engine to initialise (with fallback if not available)
  await page.waitForFunction(
    () => window.particleEngine != null || document.querySelector('[data-testid="sim-canvas"]') != null,
    { timeout: 8_000 },
  );
  ...
  await page.evaluate(() => {
    ...
    if (simProgress) {
      ...
      simProgress.style.gap           = 'var(--space-xs, 8px)';
    }
  ...
  // Try to spawn particles if particle engine is available
  await page.evaluate(() => {
    ...
    if (engine._nodePos) {
      ARCHETYPE_IDS.forEach((id, i) => {
        if (!engine._nodePos.has(id)) {
          ...
          const y = H * 0.22;
          if (engine.setArchPos) engine.setArchPos(id, x, y);
        }
      });
      PRODUCT_IDS.forEach((id, i) => {
        if (!engine._nodePos.has(id)) {
          ...
          const y = H * 0.80;
          if (engine.setProductPos) engine.setProductPos(id, x, y);
        }
      });
    }
  ...
  console.log('✅ Saved: loading_state_desktop.png');
});

// ── Screenshot 3: completed_state_desktop ────────────────────────────────────

test('AC12 screenshot 3: completed_state_desktop', async ({ page }) => {
  ...
  console.log('✅ Saved: completed_state_desktop.png');
});

// ── Screenshot 4: error_state_desktop ────────────────────────────────────────

test('AC12 screenshot 4: error_state_desktop', async ({ page }) => {
  ...
  console.log('✅ Saved: error_state_desktop.png');
});
